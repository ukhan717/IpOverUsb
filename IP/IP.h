...
#define IP_VERSION   33103   // Format: Mmmrr. Example: 33103 is 3.31c
...
typedef struct {
  ...
  void  (*pfCorrectTime)(unsigned IFaceId, unsigned* pFlags, U32 Sec, U32 NSec);  // Callback used by IP_PTP to give a time correction to the driver.
...
enum {
  ...
  IP_WIFI_SECURITY_WPA_ENTERPRISE,
  IP_WIFI_SECURITY_WPA2_ENTERPRISE           = IP_WIFI_SECURITY_WPA_ENTERPRISE,  // All supported modules use WPA/WPA2 combined, so everything is the same.
  IP_WIFI_SECURITY_WPA_WPA2_MIXED_ENTERPRISE = IP_WIFI_SECURITY_WPA_ENTERPRISE   // All supported modules use WPA/WPA2 combined, so everything is the same.
};
...
void IP_DHCPC_SetVendorOptionsCallback(IP_DHCPS_GET_VENDOR_OPTION_FUNC* pf);
...
#define IP_PTP_FLAGS_COARSE_CORRECTION_MASK  (1uL << 1)  // Coarse correction is used instead of fine.
#define IP_PTP_FLAGS_FINE_RESET_MASK         (1uL << 2)  // Fine addend dicotomy caluclated values are reset.
...
typedef struct {
  ...
  U8  Flags;        // ORR-ed combination of IP_PTP_FLAGS_*
} IP_PTP_CORRECTION_INFO;

typedef void (*IP_PTP_pfOnCorrection)(IP_PTP_CORRECTION_INFO* pInfo);
...
void IP_PTP_OC_SetOnCorrectionCallback(IP_PTP_pfOnCorrection pf);
